        ...
        IClassFixture<MastersDataWebApplicationFactory<Program>>
    {
        ...
        public async Task CreateOperationRequest_ReturnsSuccessStatusCode()
        {
            ...
            using (var scope = _factory.Services.CreateScope())
            {
                ...
                {
                    _output.WriteLine($"OperationType: {operationType.Id.AsString()}");
                ...
                if (patient != null)
                {
                    _output.WriteLine($"Patient: {patient.Id.AsString()}");
                ...
                if (doctor != null)
                {
                    _output.WriteLine($"Doctor: {doctor.Id.AsString()}");
            ...
            {
                throw new InvalidOperationException("Operation Type invalid.");
            }

            if (patientId == null)
            {
                throw new InvalidOperationException("Patient invalid.");
            }

            if (doctorId == null)
            {
                throw new InvalidOperationException("Doctor invalid.");
            }
            ...
            _output.WriteLine($"Status da resposta: {response.StatusCode}");
            // Assert
            response.EnsureSuccessStatusCode();

            var responseString = await response.Content.ReadAsStringAsync();
            Assert.Contains("2027-01-01", responseString);
