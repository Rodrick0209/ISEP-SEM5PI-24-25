...
export default class Maze {
    constructor(parameters) {
        this.onLoad = function (description) {
            ...
            this.bedPosition = this.cellToCartesian(description.bedPosition);
                ...
                for (let j = 0; j <= description.size.height; j++) { // In order to represent the southmost walls, the map height is one row greater than the actual maze height
                    ...
                    */
                    if (description.map[j][i] == 2 || description.map[j][i] == 3) {
                        wallObject = this.wall.object.clone();
                        wallObject.position.set(i - description.size.width / 2.0 + 0.48, 0.75, j - description.size.height / 2.0);
                        this.object.add(wallObject);
                    }
                    ...
                    if (description.map[j][i] == 1 || description.map[j][i] == 3) {
                        wallObject = this.wall.object.clone();
                        wallObject.position.set(i - description.size.width / 2.0 , 0.75, j - description.size.height / 2.0 + 0.5);
